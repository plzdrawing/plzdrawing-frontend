...
import { EmptyStar } from '@/assets/images';
    ...
    <View style={[tw`w-full gap-[17px] p-[17px_32px]`, { backgroundColor: colors.colors.light_gray1 }]}>
      ...
      <View style={tw`flex-row flex-wrap`}>
        {reviewKeywords.map((keyword) => (
          <View
            key={keyword}
            style={[tw`m-[3px] py-[10px] px-[20px]`, { borderWidth: 1, borderColor: colors.colors.light_gray3, backgroundColor: colors.colors.white }]}
          >
            <Txt>{keyword}</Txt>
          </View>
        ))}
      ...
      {reviews.map((review) => (
        <View
          key={review.id}
          style={[tw`flex-row items-center p-[12px]`, { borderBottomWidth: 1, borderBottomColor: colors.colors.light_gray2 }]}
        >
          <Image
            source={{ uri: review.userProfile }}
            style={[tw`w-[60px] h-[60px] mr-[17px]`, { backgroundColor: colors.colors.light_gray3 }]}
          />
