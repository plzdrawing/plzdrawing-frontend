...
export const TalkItem: React.FC<TalkItemProps> = ({
  ...
  return (
    <TouchableOpacity
      onPress={onClickTalk}
      style={tw`flex-row w-full p-[12px] border-b border-light-gray-2`}
    >
      <View style={tw`relative mr-[17px]`}>
        ...
        {unreadCount > 0 && (
          <View
            style={tw`absolute top-[-6px] left-[-6px] bg-main-yellow rounded-[10px] w-[15px] h-[15px] justify-center items-center`}
          >
      ...
      <View style={tw`flex-1`}>
        <View style={tw`flex-row items-center gap-[7px] mb-[7px]`}>
          <Txt variant='bodyTextBold'>{userName} 님</Txt>
        ...
        <View style={tw`flex-1`}>
          <Txt variant='auxiliaryTextLight' numberOfLines={2}>
            {lastMessage}
          </Txt>
        </View>
      </View>
